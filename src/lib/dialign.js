 ...
 */
dialign.alignSeqFile = (inputFile, outFormat, callback) => {
 ...
 */
dialign.alignSeqString = (input, outFormat, callback) => {
...
function alignSequence(type, inputFile, outFormat, callback) {
    if (fs.existsSync(inputFile)) {
        ...
        const dialignCommand = `./dialign2-2 ${outFormat}-fn ${tempOutputFile} ${path.resolve(inputFile)}`;
        run(dialignCommand, err => {
            if (err) {
                return callback(err);
            } else {
                let data = '';
...
function run(command, callback) {
    let execPath = dialign.execLocation;
    let envPath = dialign.envVariablePath;
